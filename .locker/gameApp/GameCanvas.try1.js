...
class GameCanvas extends Component {
  ...
  render() {
    ...
    if (window.createjs) {
      ...
      //this.stage = new window.createjs.Stage('gameCanvas');
      ...
      this.stage.addChild(circleB);
      this.stage.addChild(circleC);
      this.stage.addChild(circleD);
      this.stage.addChild(circleE);
      this.stage.addChild(circleF);
      ...
      function handleTick() {
        ...
        circle.y += 10;
        // Will cause the circle to wrap back
        if (circle.y > this.stage.canvas.height) {
          circle.y = 0;
        }
        // Circle will move 10 units to the right.
        circleB.y += 10;
        // Will cause the circle to wrap back
        if (circleB.y > this.stage.canvas.height) {
          circleB.y = 0;
        }
        // Circle will move 10 units to the right.
        circleC.y += 10;
        // Will cause the circle to wrap back
        if (circleC.y > this.stage.canvas.height) {
          circleC.y = 0;
        }
        // Circle will move 10 units to the right.
        circleD.y += 10;
        // Will cause the circle to wrap back
        if (circleD.y > this.stage.canvas.height) {
          circleD.y = 0;
        }
        // Circle will move 10 units to the right.
        circleE.y += 10;
        // Will cause the circle to wrap back
        if (circleE.y > this.stage.canvas.height) {
          circleE.y = 0;
        }
        // Circle will move 10 units to the right.
        circleF.y += 10;
        // Will cause the circle to wrap back
        if (circleF.y > this.stage.canvas.height) {
          circleF.y = 0;
        }
        this.stage.update();
      }
